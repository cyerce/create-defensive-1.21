...
public class DynamiteEntity extends ThrowableItemProjectile {
    ...
    public DynamiteEntity(Level level, LivingEntity shooter) {
        super(DefensiveEntities.DYNAMITE.get(), shooter, level);
    }

    public DynamiteEntity(Level level, double x, double y, double z) {
        super(DefensiveEntities.DYNAMITE.get(), x, y, z, level);
    }

    protected Item getDefaultItem() {
        return DefensiveItems.DYNAMITE.get();
    }
    ...
    protected void explode(Level level){
        level.explode(this, this.getX(), this.getY(), this.getZ(), 2, false, Level.ExplosionInteraction.TNT);
    }
