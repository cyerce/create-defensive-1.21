...
public class CokingOvenMenu extends AbstractContainerMenu {
    public final CokingOvenBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public CokingOvenMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()),
                new SimpleContainerData(2));
    }

    public CokingOvenMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data){
        super(DefensiveMenuTypes.COKING_OVEN_MENU.get(), pContainerId);
        checkContainerSize(inv, 2);
        blockEntity = ((CokingOvenBlockEntity) entity);
    ...
    public ItemStack quickMoveStack(Player playerIn, int pIndex) {
        ...
        if (pIndex < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT) {
            // Player inventory -> only try input slot (slot 0)
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, DefensiveBlocks.COKING_OVEN.get());
    }
