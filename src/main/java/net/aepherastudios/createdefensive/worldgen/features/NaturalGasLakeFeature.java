...
public class NaturalGasLakeFeature extends Feature<NoneFeatureConfiguration> {
    public NaturalGasLakeFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
        ...
        for (int x = -radius; x <= radius; x++) {
            for (int y = -radius; y <= radius; y++) {
                for (int z = -radius; z <= radius; z++) {
                    ...
                    if (dist <= jitteredRadius) {
                        pos.set(origin.offset(x, y, z));
                        level.setBlock(pos, DefensiveBlocks.RAW_NATURAL_GAS.get().defaultBlockState(), 2);
                    }
                }
            }
        }
